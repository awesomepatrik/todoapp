        ...
        <template x-if="!isEdit">
            <div class="flex items-center text-sm mt-2">
                <button @click="$refs.addTask.focus()">
                    <svg class="w-3 h-3 mr-3 focus:outline-none" fill="none" stroke-linecap="round"
            ...
            <template x-for="todo in todos" :key="todo.id" x-if="!isEdit">
                <li class="flex justify-between items-center mt-3" x-show="todo.task !== ''">
                    ...
                    <div>
                        ...
                        <button type="button" @click.prevent="editTask(todo.id)">
                            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                                 class=" w-4 h-4 text-gray-600 fill-current"
                                 viewBox="0 0 48 48"
                                 style=" fill:#000000;">
...
<script>
    window.app = function () {
        return {
            ...
            editTask(id) {
                let task = this.todos.find(x => x.id === id).task;
                this.$wire.set('isEdit', true)
                this.id = id;
                this.$wire.set('task', task);
            },
            updateTask() {
                this.$wire.saveTask(this.id);
                this.$wire.set('task', '')
                this.$wire.set('isEdit', false);
            },
            cancelUpdate() {
                this.$wire.set('task', '')
                this.$wire.set('isEdit', false)
